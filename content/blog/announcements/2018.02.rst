...
:cover: {filename}/img/blog/announcements/2018.02/cover.jpg
:tags: C++, OpenGL, WebGL, WebAssembly, NaCl, Android, iOS, Windows, ANGLE, UI,
    Metal, Vulkan, Qt, CMake, Assimp, DevIL, Vcpkg
...
post, be sure to also subscribe to `the blog Atom feed <http://blog.magnum.graphics/feeds/all.atom.xml>`_
...
.. image:: {filename}/img/blog/announcements/2018.02/opengl-search.png
    :alt: OpenGL symbol search
    :target: http://doc.magnum.graphics/magnum/#search
...
runs on the web and the :dox:`magnum-info` / :dox:`magnum-al-info` utilities
were ported to provide information about your WebGL implementation as well.
...
the `UTF-8 Manifesto <http://utf8everywhere.org/>`_, all Magnum APIs were
...
:dox:`OpenGLTester` library and it gained support for
:dox:`GPU time benchmarks <OpenGLTester-benchmarks>`. For easier testing on
...
of :dox:`Buffer::data()` and :dox:`Texture::image()` on those platforms.
...
supports :dox:`many new buffer formats <Audio::Buffer::Format>`. The HRTF
support is accompanied with a new example and the whole :dox:`Audio` library
was updated to work on the web as well:
...
that's handled with :dox:`Context::resetState()`. It's also possible to wrap
externally created OpenGL objects using :dox:`*::wrap() <Buffer::wrap()>` and,
conversely, release their ownership with :dox:`release() <Buffer::release()>`.
A reduced example showing `rendering into QQuickFramebufferObject <http://blog.qt.io/blog/2015/05/11/integrating-custom-opengl-rendering-with-qt-quick-via-qquickframebufferobject/>`_:
...
See :dox:`the documentation for more information <Context-command-line>`.
...
--- it integrates the `DART Dynamic Animation and Robotics Toolkit <http://dartsim.github.io/>`_
